...
class XMLReader(object):
    ...
    '''
    def __init__(self, character_delimiter=None, make_lower_case=True):
        '''
        Constructor
        
        Args:
            character_delimiter (str): If the XML annotation contains special
                characters and the these characters are seperated by a special
                delimiter, this delimiter can be given here.
                E.g.: 'c-o-n-v' is annotation then '-' would be the delimiter.
            make_lower_case (bool): whether to convert XML annotation values to
                lower case
        '''
        self.logger = logging.getLogger('XMLReader')
        self.character_delimiter = character_delimiter
    ...
    def load_train_test_xml(self, train_xml_path, test_xml_path, img_dir):
        ...
        if self.character_delimiter is not None:
            self.logger.info('Using character delimiter: %s', self.character_delimiter)
    ...
    def load_word_list_from_READ_xml(self, xml_filename, img_dir):
        ...
        for word_idx, word_elem in enumerate(root.findall('spot')):
            ...
            if self.character_delimiter is not None:
                transcription = transcription.split(self.character_delimiter)
